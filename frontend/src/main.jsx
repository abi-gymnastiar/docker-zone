...
function App() {
  const serviceName = location.pathname.startsWith("/service/")
              ...
              <div className="buttons">
                {service.actions?.map((action) => (
                  <button
                    key={action}
                    disabled={
                      (action === "start" && service.running) ||
                      (action === "stop" && !service.running)
                    }
  ...
  return (
    <main>
      <header>
        <h1>★ MY DOCKER ZONE ★</h1>
        <p>tiny control panel / very serious technology</p>
      </header>
      <section className="panel">
        <h2>CONTAINERS</h2>
        {services.map((item) => (
          <a className="service" href={`/service/${item.name}`} key={item.name}>
